        ...
        if(in_array($fileActualExt, $valid_extensions)) {	
		    ...
		    if ($error == 0) {
                if ($filesize <= 400*1024) {
   		          ...
   		          $fileDestination = $_SERVER['DOCUMENT_ROOT'].'Blog_Kinyarwanda/private/assets/image/users/user_image_profile/'.$fileName;	
   		         ...
   		         if(move_uploaded_file($fileTmpName,$fileDestination)) {
		         	...
		         	if ($sql->num_rows > 0)
		         	   exit("User information Already Exists!");
		                 ...
		                 $db->query("INSERT INTO users (username,firstname,lastname,email,password,date,cover_image,profile_image,address,country,state) 
		         		VALUES ('$username','$firstname','$lastname','$email','$password','$date','$coverName','$fileName','$address','$country','$state')");
